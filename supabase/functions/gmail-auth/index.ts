...
async function handleStart(req: Request, body: Record<string, string>): Promise<Response> {
  ...
  await admin.from("user_gmail_settings").upsert({
    user_id: userId,
    oauth_state: state,
...
async function handleCallback(req: Request): Promise<Response> {
  ...
  if (error) {
    if (appOrigin) {
      return new Response(null, {
        status: 302,
        headers: { Location: `${appOrigin}/settings?error=gmail` },
      });
    }
  ...
  await admin.from("user_gmail_settings").update({
  ...
  if (appOrigin) {
    return new Response(null, {
      status: 302,
      headers: { Location: `${appOrigin}/settings?connected=gmail` },
    });
  }
...
async function handleRefresh(req: Request): Promise<Response> {
  ...
  if (!tokenRes.ok) {
    ...
    return new Response(JSON.stringify({ error: "Refresh failed", disconnected: true }), { status: 400, headers: corsHeaders });
  }
...
async function handleDisconnect(req: Request): Promise<Response> {
  ...
  // Delete cached emails and settings
  await Promise.all([
    admin.from("email_cache").delete().eq("user_id", userId),
    admin.from("user_gmail_settings").delete().eq("user_id", userId),
  ]);
...
<div style="text-align:center;padding:2rem;">
  ...
  <h1 style="font-size:1.25rem;color:#111827;margin-bottom:0.5rem;">Gmail Connected</h1>
